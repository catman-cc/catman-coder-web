 ...
 *  用于存放循环引用数据的上下文
 */
export interface LoopReferenceContext {
...
export interface TypeDefinitionSchema {
  root: string;
...
export interface ValueDefinition extends Base {
  kind: string; //值类型
}
...
export interface FunctionInfo extends Base {
  id: string;
  name: string;
  kind: string;
  args: {
...
export interface JobDefinition extends Base {
  id: string;
  name: string;
...
export type ComponentCreatorFunction = (
  node: LayoutNode<unknown>
) => React.ReactNode;
export interface ComponentCreator {
  support(_node: LayoutNode<unknown>): boolean;
  create(_node: LayoutNode<unknown>): React.ReactNode;
...
export interface LayoutContext {
   ...
   * @param type 可选的展示类型
   */
  createOrActive(node: LayoutNode<unknown>, type?: LayoutType): void;
...
export interface EventBusContext {
  // 触发事件
  publish(event: Event<unknown>): EventBusContext;
  // 订阅事件
  addListener(listener: EventListener): EventBusContext;
  subscribe(
...
export interface ResourceService {
  root(_selector?: string): Promise<API.Response<Resource>>;
  findById(_id: string): Promise<API.Response<Resource>>;

  save(_resource: Resource): Promise<API.Response<Resource>>;

  rename(_id: string, _name: string): Promise<API.Response<Resource>>;

  delete(_resource: Resource): Promise<API.Response<Resource>>;
  /**
  ...
  create<T>(_resource: Resource): Promise<API.Response<ResourceDetails<T>>>;
  move(
    _id: string,
    _parentId?: string,
    _previousId?: string,
...
export type MenuRenderFunction = (
  menu: Menu<unknown>,
  _resource: Resource
) => JSX.Element;
export type MenuItemRenderFunction = (
  _menu: Menu<unknown>,
  _resource: Resource,
  _item: ReactNode
) => JSX.Element;
...
export interface ResourceMenuContext {
   ...
   * @param handler 处理器
   */
  process(handler: (context: ResourceMenuContext) => void): void;

  render(resource: Resource): ReactNode;

  showMenus(resource: Resource): void;
}
...
export interface Channel {
  ...
  subscribe(matcher: MessageMatcher, handler: MessageHandler<unknown>): Channel;
