import { ComplexType, Scope } from "@/common/core"
...
export type TypeDefinitionHierarchialSchemaWatcher = (_schema: TypeDefinitionHierarchialSchema) => void


export class TypeDefinitionHierarchialSchema {

    schmea: Core.TypeDefinitionSchema
    ...
    static of(schema: Core.TypeDefinitionSchema) {
        const parse = TypeDefinitionSchemaParser.parse(schema)
        return new TypeDefinitionHierarchialSchema(parse.newSchema, parse.registry, parse.idMapper, new DefaultIdMapper(), [])
    }

    static copy(hierarchial: TypeDefinitionHierarchialSchema) {
        return new TypeDefinitionHierarchialSchema({
            ...hierarchial.schmea
        }, hierarchial.registry, hierarchial.idMapper, hierarchial.outIdMapper, hierarchial.watchers)
    }

    constructor(schema: Core.TypeDefinitionSchema, registry: HierarchicalRegister, idMapper: IdMapper, outIdMapper: IdMapper, watchers: TypeDefinitionHierarchialSchemaWatcher[]) {
        this.schmea = schema
     ...
     * @param index 添加的位置
     */
    add(pid: string, schema: Core.TypeDefinitionSchema, index: number = 0, outId?: string) {
    ...
    addBrother(id: string, schema: Core.TypeDefinitionSchema, before: boolean = false) {
    ...
    getParent(id: string): Core.TypeDefinition | undefined {
        const pid = this.findParent(id)
        if (pid) {
            return this.get(pid)
        }
    ...
    containPosterity(id: string, posterityId: string): boolean {
        ...
        return this.getChildren(id).some(c => this.containPosterity(c, posterityId))
    }
    ...
    toTypeDefinitionSchema(id?: string): Core.TypeDefinitionSchema {
...
export class TypeDefinitionSchemaParser {
    ...
    private parse(schema: Core.TypeDefinitionSchema) {
