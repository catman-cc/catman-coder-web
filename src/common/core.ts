...
export class DefaultTypeDefinition implements Core.TypeDefinition {
    ...
    recursionChild(): DefaultTypeDefinition[] {
        const ncs = [...this.type.items, ...this.type.items.map((c: DefaultTypeDefinition) => {
            return c.recursionChild()
        })];
        return Array.isArray(ncs) ? ncs as DefaultTypeDefinition[] : [ncs]
    }

    recursionChildWithCallback(callback: (_child: DefaultTypeDefinition, _parent: DefaultTypeDefinition) => void) {
    ...
    static ensure(typeDefinition: Core.TypeDefinition): DefaultTypeDefinition {
        return typeDefinition instanceof DefaultTypeDefinition ? typeDefinition : new DefaultTypeDefinition(typeDefinition)
    }
...
export class ComplexType implements Core.Type {
    ...
    isRaw(): boolean {
        return this.getTypeName() === constants.Types.TYPE_NAME_STRING
    ...
    isRefer(): boolean {
        return this.getTypeName() === constants.Types.TYPE_NAME_REFER || this.getTypeName() === constants.Types.TYPE_NAME_STRUCT;
    }
    isMap(): boolean {
