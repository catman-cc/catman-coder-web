...
import { Resource } from "catman-coder-core";
...
export const RootResourceQuery = createAsyncThunk("resource/root", async () => {
...
export const ResourceSlice = createSlice({
  ...
  initialState,
  reducers: {
    save(state, action: PayloadAction<Resource>) {
      ...
      function save(resource: Resource) {
        ...
        const parent = state.resources[resource.parentId] as Resource;
        if (!parent.children) {
    ...
    remove(state, action: PayloadAction<Resource>) {
      const resource = action.payload;
      if (resource.parentId) {
  ...
  extraReducers: (builder) => {
    builder.addCase(RootResourceQuery.fulfilled, (state, action) => {
      ...
      // 递归处理
      const recursionHandlerResource = (resource: Resource) => {
