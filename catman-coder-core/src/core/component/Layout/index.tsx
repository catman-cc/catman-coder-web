...
export class NamedComponentCreator<T extends NamedNode>
  implements ComponentCreator<T>
{
  name: string;
  doCreate: (_node: T) => Product<T> | JSX.Element;

  static of<T extends NamedNode>(
    name: string,
    doCreate: (_node: T) => Product<T> | JSX.Element
  ) {
...
export class DefaultFactory<T extends NamedNode> {
  ...
  nameMatch(
    name: string,
    createMethod: (_node: T) => Product<T> | JSX.Element
  ) {
