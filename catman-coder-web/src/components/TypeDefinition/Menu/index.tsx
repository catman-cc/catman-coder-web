// @flow
import { FuzzyQuery } from "catman-coder-core";
import { ComplexType, DefaultTypeDefinition, Scope } from "catman-coder-core";
import { EventBus } from "catman-coder-core";
...
import {
  TypeDefinitionListQuery,
  TypeDefinitionSlice,
} from "@/stores/typeDefinitions";
...
import { DefaultLayoutNode } from "catman-coder-core";
...
const TypeDefinitionMenu = (props: Props) => {
  ...
  const [tds, setTds] = React.useState<TypeDefinition[]>([]);

  const tdsComput = () => {
    const all: TypeDefinition[] = [];
    if (query.key === "") {
    ...
    all.push(
      ...tdStore.tds.filter((td) => {
        const tdid = td.id || "";
        const fields =
          query.fields.length > 0
            ? query.fields
            : ["id", "name", "describe", "labels", "alias"];
   ...
   * @returns  菜单项
   */
  const toMenuItem = (td: TypeDefinition): ItemType => {
    return {
      key: td.id!,
      icon: (
        <IconCN
      ...
      label: (
        ...
        >
          <div className="type-definition-menu-label">
            <div className="td-name">{td.name}</div>

            <div className="td-id">id: {td.id}</div>
          </div>
        </Popover>
      ),
    };
          ...
          items={[
            {
              ...
              label: (
                <TitleBar>
                  <IconCN type="icon-moxing" /> 数据类型定义
                  <div
                    style={{
                      display: "flex",
                      flexWrap: "nowrap",
                      ...
                      content={
                        ...
                        >
                          ...
                          <Search
                            ...
                            placeholder="模糊查询,支持正则"
                            onSearch={() => {}}
                            enterButton={
                              <Button
                                icon={
                                  <IconCN
                                    type="icon-a-qingkong3x"
                                    onClick={() => {
                                      setQuery({
                                        fields: [],
                                        key: "",
                      ...
                      <HiDocumentAdd
                        ...
                        onClick={(e: Event) => {
                          ...
                          const ntd = DefaultTypeDefinition.create({
                            scope: Scope.PUBLIC,
                            type: ComplexType.ofType(
                              constants.Types.TYPE_NAME_STRING,
                            ),
                          }).unWrapper();
...
//      * @param td 类型定义
//      */
//     toMenuItem(td: DefaultTypeDefinition): ItemType {
//         return {
//             key: td.id,
