...
import { Avatar, Badge, Button, Card, Divider, Drawer, DrawerProps, Dropdown, Space, Switch, Tag } from "antd"
...
const Editor = (props: Props) => {
    ...
    const [tree, setTree] = useState(TypeDefinitionTree.of(DefaultTypeDefinition.ensure(props.td)));
    ...
    useEffect(() => {
        ...
        setDrawerView(
            { ...drawerView!, config: { ...drawerView?.config, title: title() } }
        )
    }, [tipIndex])
    ...
    const loadFromCache = (id: string | undefined, simple: boolean) => {
        ...
        if (!cache[rid]) {
            ...
            cache[rid] = simple ? <ReactJson src={obj} /> : <VanillaJSONEditor
    ...
    return <div >
        <Row>
            ...
            <div>
                ...
                >
                    <Avatar style={{ backgroundColor: '#1677ff' }} icon={<CodeSandboxOutlined />} />
            ...
            <ToolBar>
                ...
                <Dropdown.Button
                    ...
                    onClick={() => {
                        ...
                        props.node.getModel().doAction(Actions.renameTab(props.node.getId(), obj.name))
                    }}
                    menu={{
                        items: [
                            {
        ...
        <Drawer
            title={
                <div>
                    <Tag>JSON FORMAT</Tag> <Switch checkedChildren="简单" unCheckedChildren="复杂"
                    ...
                    <Button
                        onClick={() => {
                            EventBus.emit(Events.Layout.ADD_TAB, {
                                ...
                                "config": {
                                    "data": JSON.stringify(convert(tree.toObject(jsonView.itemId))),
                                },
                                "enableClose": true,
                                className: "TypeDefinitionMenuIcon",
                                "icon": "icon-type",
        ...
        >
            ...
            {
                jsonView.show ? loadFromCache(jsonView.itemId, jsonView.simple || false) : <div></div>
            }
        </Drawer>
