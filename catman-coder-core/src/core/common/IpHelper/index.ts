 ...
 */
export const validateIP = (ip: string): number => {
  // 利用正则表达式检查 IP 地址的格式
  const ipRegex =
    /^\b(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\/(?:[0-9]|[1-2][0-9]|3[0-2]))?\b$/;
 ...
 * @returns  中文
 */
export const convertIPAddressType = (type: IPAddressType): string => {
  switch (type) {
    case "INNER":
      return "内网地址";
    case "PUBLIC":
 ...
 * @returns  类型
 */
export const getIPAddressType = (ip: string): IPAddressType => {
  // 分割IP地址和子网掩码
