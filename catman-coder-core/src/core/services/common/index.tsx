...
export class AntDesignProHelper {
  static convertAntDesignProPage(
    params: ParamsType & {
      pageSize?: number;
      current?: number;
      keyword?: string;
...
export function convertAntDesignProPage(
  ...
  if (sort) {
    ...
    Object.keys(sort).forEach((key) => {
      const v = sort[key];
      const sortValue = v === null ? "ASC" : v === "ascend" ? "ASC" : "DESC";
...
export type PageFunction<T> = (
  _page: API.Page
) => Promise<API.Response<API.VPage<T>>>;

export function doPage<T>(
  params: ParamsType & {
    pageSize?: number;
    current?: number;
    keyword?: string;
  ...
  return request(convertAntDesignProPage(params, sort)).then((res) => {
    const vp = res.data;
    return {
      data: vp.records,
      success: res.success,
