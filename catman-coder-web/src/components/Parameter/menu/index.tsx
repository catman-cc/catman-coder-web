...
import { EventBus, Events } from "catman-coder-core";
...
const ParameterMenu = (props: Props) => {
   ...
   * @returns  菜单项
   */
  const toMenuItem = (param: Parameter): ItemType => {
    return {
      key: param.id!,
      icon: (
        <IconCN
      ...
      label: (
        ...
        >
          <div className="type-definition-menu-label">
            <div className="td-name">{param.name}</div>

            <div className="td-id">id: {param.id}</div>
          </div>
        </Popover>
      ),
    };
