...
export class ResourceProcessor implements Processor {
  ...
  run(context: ApplicationContext) {
    context.events.watchByName(
      Constants.EventName.newTypeDefinition,
      (event, eventBus) => {
      ...
      ...([
        {
          ...
          onMenuClick: (
            ...
            context.resourceContext!.service!.flush(group.id).then((res) => {
              const resourceDetails = res.data;
              // 处理资源
              context.events?.publish({
                id: "resource-reload",
